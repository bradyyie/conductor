...
**Note on retiring `excludeFilters` (D3):** the `SystemTaskRegistry.byType` + `isOverride()` seam is in place, but each excluded class has *bespoke* Orkes replacement semantics (e.g. WAIT is excluded because Orkes replaces the inline `Wait` system task with `OrkesWaitTaskMapper` + queue/timer-based `OrkesWaitTimer`; SubWorkflow→`SubWorkflowSync`; mappers/event-processor/queue-manager each differ). Retirement is therefore a careful per-bean initiative (give each Orkes override a distinct `@Component` name + `isOverride()=true`, then drop it from excludeFilters), NOT a mechanical sweep — deferred to avoid destabilizing the green state.
