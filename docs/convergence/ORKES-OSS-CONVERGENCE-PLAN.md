...
### Phase 4 — monolith split ✅ COMPLETE: base modules + gateway/human/registry/integration/scheduler (13 → 3; the 3 remaining accepted as legitimate cross-feature edges, guardrail now enforcing fail-on-new + wired into CI)
...
**CURRENT STATE (2026-06-26): module-split burn-down ✅ COMPLETE. 13 → 3 cross-feature edges, and the remaining 3 are ACCEPTED as legitimate. Guardrail flipped to ENFORCING + wired into CI.**
The 13 P1 sideways violations are resolved: 10 eliminated (gateway/human/registry/scheduler store moves + the integration unused-dep removal), and the final **3 are reviewed and accepted as legitimate cross-feature edges** (owner call), not tech debt:
- `api-gateway → integration` — gateway schema validation calls integration's concrete `SchemaService`/`SchemaReferenceService`.
- `event-integration → integration` — enterprise event workers (incl. `MessageBroker`) use integration's concrete `IntegrationService`.
- `event-processor → event-integration` — event processing consumes the `MessageBroker`/`OnMessageHandler`/`MessageBrokerType` contracts, which are themselves integration-coupled (`MessageBroker` imports `IntegrationService`), so they cannot be lifted to `core`.
Investigation confirmed none is an unused dep or a clean port-extraction (and `-api` extraction was already tried + reverted). So instead of forcing concrete service classes into `core` (which would invert layering), the `moduleDependencyReport` guardrail now **enforces fail-on-new**: these 3 are encoded as `allowedSidewaysEdges` in root `build.gradle` with rationale; **any other** feature→feature edge fails the build. Verified by temporarily de-listing one edge (→ `BUILD FAILED [NEW]`) and restoring (→ green: legitimate 3, NEW 0). Wired into `cicd_v2.yaml` (runs after `spotlessCheck`, before `build`). **The persistence/module-graph convergence (§8 Phase 4) is effectively done; remaining work is the non-persistence backlog (Phase 2 engine backports, `OrkesWorkflowExecutor extends WorkflowExecutorOps`, Phase 5/7 ops).**

**Scheduler (DONE — 2 edges eliminated, 5 → 3). Detail of the completed work below:**
- **scheduler (2 edges) — prod move is trivial; the TEST re-homing is a real refactor needing an approach decision.** Deep investigation (2026-06-26) established:
  - **Prod is self-contained & easy.** Only 3 prod files reference scheduler-enterprise types: `dao/postgres/scheduler/PostgresSchedulerDAO`, `dao/postgres/archive/PostgresSchedulerArchiveDAO`, `dao/postgres/archive/SecurePostgresSchedulerArchiveDAO` (extends the archive DAO); MySQL: `dao/mysql/scheduler/MySQLSchedulerDAO`. Moving these to `scheduler-enterprise` keeping their packages keeps the server's string-literal ref (`MySQLModePostgresExclusionFilter`) valid with zero import churn.
  - **Test-canonical question RESOLVED by evidence (corrects earlier "stale" note):** `scheduler/service/*` copies in postgres-persistence ARE stale subsets of scheduler-enterprise's canonical suite (`SchedulerServiceTest`: 0 unique methods vs +16 in se; `PostgresSchedulerArchiveDAOTest`: identical method set) → safe to delete. BUT `dao/postgres/metadata/PostgresSchedulerDAOTest` is **UNIQUE, high-value coverage** (~40 DAO-level tests: `testBuildSortClause_*`, `testSearchSchedulesWithFreeText_*`, sort-by-paused, 97 RBAC/Subject/Access/Tag refs) NOT replicated in scheduler-enterprise (which only tests `searchSchedules` at the service level). Must be preserved, not deleted.
  - **The real barrier:** the unique PG test extends postgres-persistence's **shared** `dao/postgres/AbstractTestDAO` (base for ~35 tests; it hard-instantiates `schedulerDAO` at line 183 — used by NO non-scheduler test, so removable). Moving the unique test out is forced (else postgres-persistence test→scheduler-enterprise + the existing scheduler-enterprise test→postgres-persistence = a **project cycle**). But **this repo uses NO `java-test-fixtures` anywhere** — test classes are not shareable across modules (which is exactly why scheduler-enterprise already keeps duplicate `AbstractTestDAO`/`PostgresDAOTestUtil`/`Mock*` copies). So the unique 97-RBAC-ref test can't simply extend the postgres base from scheduler-enterprise; it must be re-homed onto scheduler-enterprise's own (scheduler-focused) base — which lacks the security/tag/RBAC infra it needs — or test-fixtures must be introduced. **MySQL is worse:** scheduler-enterprise has **zero** MySQL test capability; `MySQLSchedulerDAOTest extends MySQLDAOTestBase` (a mysql-persistence test class), so the move requires standing up MySQL Testcontainers + base in scheduler-enterprise from scratch (or test-fixtures).
  - **Decision (owner):** split the unique DAO test along RBAC lines — RBAC-dependent coverage stays Orkes-only; the non-RBAC DAO-logic coverage becomes the OSS-style tests. Then move prod + re-home the split tests into `scheduler-enterprise`.
  - **POSTGRES half — DONE (5→4), green & verified.** Moved the 3 prod files (`PostgresSchedulerDAO`, `PostgresSchedulerArchiveDAO`, `SecurePostgresSchedulerArchiveDAO`) to `scheduler-enterprise` keeping packages (server string-ref intact); scheduler-enterprise prod gained only `implementation postgres-base` (DAOs' `dao.postgres.{base,util}` resolve to postgres-base, config classes to core/common — no new prod edge). Stripped the unused `schedulerDAO` field/instantiation from the shared `postgres-persistence` `AbstractTestDAO` (the cycle source). Split the unique `PostgresSchedulerDAOTest` (43 tests) via brace-counting into `PostgresSchedulerSearchDAOTest` (32 non-RBAC, extends scheduler `AbstractTestDAO`) + `PostgresSchedulerRbacDAOTest` (11 RBAC/tag, extends new thin `AbstractRbacTestDAO` that wires `tagsDAOV2`+`rbacDAO`); re-homed the 2-CRUD `SchedulerDAOTest`; deleted the 7 stale `scheduler/service/*` subset copies. Dropped `postgres-persistence → scheduler-enterprise` prod **and** test edges. Verified: both modules' main+test compile; **45/45** scheduler tests pass (32+11+2); shared harness intact (`EnvironmentDAOTest` green); `moduleDependencyReport` 5→**4**.
  - **MYSQL half — DONE (4→3), green & verified.** Simpler than Postgres: `MySQLSchedulerDAOTest` is self-contained (wires its own `schedulerDAO`; "RBAC" refs are just `Subject`/`Access` args to `getAllSchedulesWithPermissions`, no separate `rbacDAO`/`tagsDAO`), so **no split needed** — moved wholesale. Moved `MySQLSchedulerDAO` to scheduler-enterprise (+ `implementation mysql-base`; its `SchedulerArchivalDAO`/`SearchSchedulerExecutionsQuery` resolve to scheduler-oss/core already on the classpath — no new prod edge). `MySQLDAOTestBase` is shared by 39 mysql tests so it was **copied** (repo has no test-fixtures); scheduler-enterprise test gained `testImplementation mysql-persistence` (for `MySQLConfiguration` + `db/migration`; distinct from Postgres' `db/migration_archive_postgres`, no Flyway collision) + testcontainers:mysql/junit-jupiter + flyway-mysql + mariadb driver. Removed the unused `scheduler-oss` **and** `scheduler-enterprise` prod edges from mysql-persistence. Verified: compile clean; **80/80** `MySQLSchedulerDAOTest` pass; mysql-persistence intact (`MySQLSchemaDAOTest` 11/11); `moduleDependencyReport` 4→**3**.
  - **SCHEDULER VERTICAL COMPLETE: 5 → 3.** Remaining 3 sideways violations are exactly the deferred feature-to-feature edges (`api-gateway → integration`, `event-integration → integration`, `event-processor → event-integration`) — still pending the product "accept+baseline vs restructure" call.
- **3 feature-to-feature edges** (`api-gateway`→`integration`, `event-integration`→`integration`, `event-processor`→`event-integration`): ✅ **RESOLVED — accepted as legitimate** (owner call; concrete services + integration-coupled broker contract; `-api` extraction reverted; not liftable to `core`). Encoded as `allowedSidewaysEdges` and the checker now **fails on any new** sideways edge (enforcement verified; wired into CI).
...
**Done:** engine cutover (`oss-core` deleted, Orkes on OSS `conductor-core`); OSS superset/seam backports; D3 excludeFilters retirement (21 task/mapper beans); Spring context loads; OSS validated repo-wide (~1771 tests, 13 modules); Orkes validated module-by-module; **org_id seam UNIVERSAL across every OSS postgres + mysql adapter — step 1 complete** (§16/§17), artifacts re-published to mavenLocal; **Phase 4 module-graph split COMPLETE — 13 → 3 sideways edges, the final 3 accepted as legitimate cross-feature edges, dependency guardrail ENFORCING (fail-on-new) + wired into CI**; **Phase 5 mechanical consolidation satisfied** (`OrgContext`/`IDTools` already single-sourced in `common`; remaining "duplicates" are diverged copies = discretionary debt, not blocking — see item 5).

**Remaining (persistence convergence §8). The clean module-graph work is done; what's left is the heavier engine/data-model items, in priority order:**
...
2. **Phase 3 (Enterprise) — prove the full vertical.** Convert **one** Orkes store (postgres `ExecutionDAO`) from a standalone re-implementation to `extends` the OSS `PostgresExecutionDAO`, overriding `applyQueryExtensions`/`applyWriteExtensions` to inject `org_id` (read filter + insert column + `ON CONFLICT` target via `OrgContext`); **delete the duplicated SQL**. ✅ The DAO half is **DONE** (§16 Phase 3). The engine half — re-express `OrkesWorkflowExecutor` as `extends WorkflowExecutorOps` — is the **single largest remaining convergence item** and is now concretely scoped (2026-06-26):
   - **Current state:** `server/.../com/netflix/conductor/core/execution/OrkesWorkflowExecutor.java` is a **3,867-line standalone reimplementation** that `implements WorkflowExecutor` (OSS interface), NOT `extends` the OSS `WorkflowExecutorOps` (2,376 lines). 18 `@Override`s; 9 imports of OSS engine internals.
   - **Why it needs the engine-core backport (confirmed by delta density):** the enterprise deltas are **woven through the engine methods**, not confined to seam points — `OrkesWorkflowExecutor` carries ~49 idempotency refs, ~37 limits/quota refs, ~24 org-context (`OrkesRequestContext`) refs, ~13 rbac/authz refs, and ~2× the OSS logic around `decide` (33 vs 19), `updateTask` (25 vs 13), `startWorkflow` (7 vs 3), `rerun` (12 vs 5). The existing OSS seams (`beforeStartWorkflow`, `onDecide`, `startWorkflowIdempotent`) cover the start/decide entry points but **not** the idempotency/limits/updateTask injection points the Orkes engine needs.
   - **Roadmap (dedicated effort, not a tail-task; the workflow engine is the most critical path):** (a) diff `OrkesWorkflowExecutor` vs OSS `WorkflowExecutorOps` and enumerate every delta; (b) for each, add a `protected` seam to OSS `WorkflowExecutorOps` (idempotency hook, limits/quota check hook, `updateTask`/`completeTask` hooks, sync-exec hook) — additive, no-op by default, byte-identical OSS behavior when not overridden; (c) republish OSS `conductor-core`; (d) rewrite `OrkesWorkflowExecutor` to `extends WorkflowExecutorOps` overriding only the seams (3,867 → a few hundred lines), deleting the duplicated engine logic; (e) full engine integration testing (sync exec, idempotency, decide loop, rerun, terminate, updateTask). Acceptance: Orkes workflow e2e green with org/limits/idempotency applied via seams, duplicated engine code gone.
   - **▶ STEP A delta analysis DONE (2026-06-26) — refines the roadmap and surfaces a gating decision.** Method-level map: **26 shared** method names (candidate deltas: `startWorkflow`, `startWorkflowIdempotent`, `decide`/`decideWithLock`, `updateTask`, `terminate(Workflow)`, `rerun`, `retry`, `restart`, `resume`/`pauseWorkflow`, `skipTaskFromWorkflow`, …), **50 Orkes-only** (enterprise additions: `_startWorkflow`/`_decide` private cores, `checkExecutePermissionsOverTask`, `validateWorkflowIdempotency`, `handleFailOnRunningIdempotencyStrategy`, and a much richer rerun family `handle{Switch,LoopOver,ForkJoinDynamic,Default}TaskRerun`/`prepareWorkflowForRerun`/`finalizeRerun`), **25 OSS-only** (incl. the `beforeStartWorkflow`/`onDecide` seams + helpers like `createWorkflowModel`/`resolveWorkflowDefinition`/`notify{Task,Workflow}StatusListener`). The start flow reads cleanly as enterprise-wrapper → core: `startWorkflow` (CacheConfig **short-circuit** dedup + notifications) → private `startWorkflow` (**RBAC** `EXECUTE` check) → `_startWorkflow` (**org_id**; **4 limits gates**: SKU read-only, archival-size read-only, `maxWorkflowsRunning` decider-queue, `maxWorkflowDuration` timeout; **Orkes idempotency** `RETURN_EXISTING/FAIL/FAIL_ON_RUNNING`) → core create/persist/decide. Enterprise collaborators injected (absent from OSS): `OrkesPermissionEvaluator`/`UserRepository`, `ConductorLimitsDAO`/`OCUMeasurement`/`WorkflowRateLimiterDAO`, `IdempotencyDAO`/`WorkflowExecutionCache`/`TaskExecutionCache`, `WorkflowExecutionMonitor`/`PeerNotificationService`, `SchemaService`.
   - **⚠ GATING FINDING:** two concerns are **pervasively woven**, not seam-localizable: **`WorkflowIntrospection` (50 refs)** wraps logic blocks throughout via `try(var record = WorkflowIntrospection.record(workflow, PHASE))`, and a **divergent idempotency model (59 refs)** (`IdempotencyDAO` + `WorkflowExecutionCache` + `IdempotencyStrategy` + cache-config dedup) that does NOT match OSS's `startWorkflowIdempotent`/`validateIdempotentWorkflowOwnership`. So a clean "extends + override a handful of seams" reparent is **gated on first decoupling introspection (→ AOP/decorator or coarser boundaries) and reconciling the idempotency models** (backport Orkes's into OSS, or keep as a full override). The cleanly-seamable concerns (org / the 4 limits gates / RBAC / cache short-circuit / notifications) are the easy part; introspection + idempotency are the hard gate.
   - **Approach options (decision needed before more code):** **(A) full decouple-then-reparent** — first lift introspection to a cross-cutting concern and converge idempotency in OSS, then reparent; biggest payoff (3,867 → few hundred) but multi-week and touches the most critical path. **(B) partial reparent** — `extends WorkflowExecutorOps`, seam-share org/limits/RBAC/notifications, but keep `startWorkflow`/`decide`/`updateTask`/idempotency/rerun + introspection as full overrides; deletes only the truly-identical inherited helpers (smaller, safer reduction). **(C) defer** — leave as-is; it is already correct and the module graph (the convergence's headline) is done.
   - **▶ DECISION (owner): (A) full decouple-then-reparent, reframed — "backport as much as possible (bar enterprise features) into OSS `WorkflowExecutorOps`, then just thin out `OrkesWorkflowExecutor` to `extends` it."** So the non-enterprise engine logic becomes canonical in OSS (the fork's `conductor-core`); enterprise features (org_id, limits/SKU/OCU, RBAC, the Orkes idempotency model incl. cache/`IdempotencyDAO`, `WorkflowIntrospection`, notifications, `SchemaService`, `WorkflowConsistency.SYNCHRONOUS`) stay in Orkes as overrides/seam-impls.
   - **▶ STEP A2 backport inventory (per-method diff, 2026-06-26) — the non-enterprise CORE-DIFFs to port Orkes→OSS**, in priority/risk order (✅=done):
     1. `scheduleNextIteration` — stamp loop-over tasks with `loopTaskId` (1 line; OSS `TaskModel` already supports it). *safest*
     2. `findLastFailedSubWorkflowIfAny` — add `DO_WHILE` branch so retry descends to the deepest failed sub-workflow inside a loop. *small/self-contained*
     3. `updateTask` — record `_updateNetworkLatency` from client `_clientSendTime` (observability; additive).
     4. `terminate(priv)` — resolve `$`-prefixed `failureWorkflow` via `parametersUtils.replaceString` instead of fixed `split(".")[2]`.
     5. `restart` — allow `workflowInput`/`correlationId` overrides to re-parameterize on restart.
     6. `decide(String)` — decider-queue hygiene: remove from `DECIDER_QUEUE` when workflow missing/terminal; re-queue with backoff on lock-acquire failure.
     7. `terminateWorkflow(WorkflowModel)` — guard against starting the failure-workflow twice.
     8. `skipTaskFromWorkflow` — for composite tasks (DO_WHILE/SWITCH/FORK_JOIN[_DYNAMIC]) create SKIPPED children + the JOIN.
     9. `updateAndPushParents` — reactivate the parent chain on deep retry (re-`retry` failed SUB_WORKFLOW children; re-schedule CANCELED fork/join). *highest correctness value; larger*
     10. `resumeWorkflow` — optional `restoreInputParameters` + `repairWorkflowOnResume` (requires porting the Orkes-only `WorkflowRepairer` + 2 flags).
     - Large/optional: adopt Orkes's lock-held, task-type-specific **rerun** engine (SWITCH/WAIT/DO_WHILE/FORK_JOIN_DYNAMIC/loop/nested-subwf) over OSS's recursive `rerunWF`.
     - Watch-outs (do NOT regress OSS, which is richer in places): OSS `updateTask` (extendLease, terminal guard, `taskStatusListener`, HUMAN re-queue, lazy-eval); OSS `retry(WorkflowModel)` permissive-JOIN; OSS `terminateWorkflow(String)` idempotent no-op guard; OSS `decide` (`adjustStateIfSubWorkflowChanged`, active-branch join reset, lazy-eval). Reconcile, don't overwrite.
     - Structural prep for `extends`: widen OSS `WorkflowExecutorOps` `private final` fields → `protected` (additive); Orkes `super(...)` passes the 13 core deps; add no-op `protected` seams for the enterprise injection points (start pre-checks/short-circuit, timeout-from-limits, idempotency, updateTask latency/event, validate-schema).
     - **▶ Backport PROGRESS + full code-level triage (2026-06-26):**
       - ✅ **#1 `scheduleNextIteration`** (loopTaskId stamp) + ✅ **#2 `findLastFailedSubWorkflowIfAny`** (DO_WHILE descent) — backported to OSS `WorkflowExecutorOps`; OSS engine tests green (`TestWorkflowExecutor` 55/0, `DecideLoop` 2/0, `SeamsTest` 2/0); committed (OSS `4226914fb`). **These were the cleanly-separable, low-risk backports — that phase is essentially complete.**
       - 🏢 **Reclassified as ENTERPRISE (stay in Orkes, do NOT backport):** #3 `updateTask` `_clientSendTime`/`_updateNetworkLatency` (Orkes-only magic-string client-latency observability, 0 OSS refs); #7 `terminateWorkflow` double-trigger guard (keys off `conductor.failure_workflow`, a flag set only by the Orkes enterprise SYNCHRONOUS path).
       - ⏸ **Deferred — need a prerequisite/test, not a mechanical port:** #4 `terminate` `$`-resolution (OSS `ParametersUtils` lacks `replaceString`; existing failureWorkflow test ⇒ semantic-change risk); #8 `skipTaskFromWorkflow` composite-children skip (`collectTasks()` self-duplication semantics + no OSS composite-skip test).
       - 🔶 **DIVERGED reconciliations — the genuinely hard part (each needs new targeted OSS tests before porting; do not rush onto the critical path):** #9 `updateAndPushParents` (Orkes adds a parent-reactivation loop — `retry(child)` on failed SUB_WORKFLOW children + reschedule CANCELED fork/join — AND uses synchronous `decide()` vs OSS `expediteLazyWorkflowEvaluation`; changes OSS nested-retry semantics); #6 `decide(String)` decider-queue hygiene (inside the diverged decide); #5 `restart` input/correlationId overrides (changes the `WorkflowExecutor` interface signature); #10 `resumeWorkflow` (`restoreInputParameters` + `repairWorkflowOnResume`, requires porting the Orkes-only `WorkflowRepairer` class); and the optional task-type-specific **rerun** engine.
       - **NET / next increment:** the easy backports are done. The remainder is either enterprise (stays) or diverged-reconciliation/structural work that must go hand-in-hand with the `extends` rewrite and **CI-scale engine integration testing** (container-per-`@SpringBootTest`, impractical locally — see Phase 7). Recommended sequencing for the dedicated effort: (1) for each DIVERGED backport write a targeted OSS engine test that captures the Orkes behavior, then port; (2) structural prep (widen OSS fields → `protected`, add no-op enterprise seams); (3) `OrkesWorkflowExecutor extends WorkflowExecutorOps`, override diverged/enterprise methods, delete the now-identical inherited helpers; (4) full Orkes engine integration suite. This is a focused multi-session initiative, not a tail task.

3. ~~**Phase 4 (Enterprise) — split the monoliths → the clean module graph.**~~ ✅ **DONE** (see §16 "Phase 4"). The enterprise feature stores moved out of the persistence monoliths to their features (gateway/human/registry/scheduler; integration was an unused dep), removing 10 of the 13 sideways edges; the final 3 are reviewed and **accepted as legitimate cross-feature edges** and the dependency checker now **enforces fail-on-new** (wired into CI). "api-gateway is just api-gateway." (Open sub-item, independent of the graph: part (a) "engine DAOs become thin OSS subclasses" is done only for postgres `ExecutionDAO` — see item 2; the diverged queue/metadata adapters remain standalone by design.)
...
5. **Phase 5 — finalize enterprise modules.** ⚠️ **Re-scoped after investigation (2026-06-26) — the original framing was stale/inaccurate:**
   - `OrgContext` (= `io.orkes.conductor.common.context.OrkesRequestContext`) and `IDTools` (`io.orkes.conductor.id.IDTools`) are **already single-sourced in `common`** (a foundation module). No consolidation needed; a new `enterprise-common` module is **unnecessary** — `common` already serves that role.
   - The "ID-generator duplicate" is real but **NOT a safe merge**: `TimeBasedUUIDGenerator` exists in `core` (org-AWARE: `generate()` prepends `orgId` via `OrkesRequestContext`, `@ConditionalOnProperty conductor.id.generator=time_based`) **and** in `workers` (org-UNAWARE plain UUID, unconditional `@Component`, **live** — injected into `EventWorker`). `workers` doesn't depend on `core`; merging would change worker event-ID format (org-prefixing) — a **product/behavioral decision**, not mechanical.
   - **Broader finding (data-driven duplicate-class survey):** cross-module "duplicates" are pervasive but **every one has diverged** (copy-paste-then-drift). Largest clusters: `workers` carries diverged copies of the `event-integration` eventing stack (`MessageBroker`, `NATS*` ×7, `Utils`, `CacheKey`, `MessageBrokerType`) and the `api-orchestration` HTTP-guard stack (`IPChecker`/`HostChecker`/`IpAddressMatcher`/… ×9); `GZipUtils` differs across `redis-persistence` (OSS-namespaced, has `zip(byte[])`) vs `archive-persistence` (AWS `IOUtils`-based). `workers` is a standalone composition-root runtime that may **intentionally** vendor these to avoid coupling — so consolidation is a per-case architecture/behavior call, **not safe-mechanical**, and is **not required for the module-graph convergence** (composition roots may depend on anything; their vendored copies are not sideways violations).
   - **Net:** the safe-mechanical part of Phase 5 is already satisfied. What remains is optional DRY hardening of divergent copies, each needing its own analysis + tests + an architecture call on whether `workers` should depend on the feature modules. Track as discretionary debt, not blocking convergence.

6. **Phase 7 / ops — CI + publishing:** run the full container-heavy server/integration suites in CI (container-per-`@SpringBootTest` is impractical locally); OSS publishing (GitHub Packages/S3); branded composite `server-enterprise`; enforce ArchUnit/dependency gates. **Progress:** the dependency gate is now enforcing (`moduleDependencyReport` fails on new sideways edges) and **wired into `cicd_v2.yaml`** (after `spotlessCheck`). Remaining here: container-heavy suites in CI + publishing.
