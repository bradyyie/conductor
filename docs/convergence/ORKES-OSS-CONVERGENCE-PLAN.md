...
This is the first enterprise store fully reparented onto an OSS adapter with `org_id` via the seam — proving the §8/§14.1 hardest path. **Next:** repeat the `extends`-OSS + override-hooks pattern for the remaining stores (higher dedup ratio where they're closer to OSS), and re-express `OrkesWorkflowExecutor extends WorkflowExecutorOps`.
