...
**State:** OSS enablers committed + published; the Orkes-side Pass-A diff was reverted to keep `orkes-conductor` green (the 3 blockers above, esp. #3, are the runway). Next focused pass: resolve #1 (flyway align) + #2 (excludeFilters), decide #3, then redo Pass A + Pass B (delete the 7 helpers + 5 inherited methods, retarget call sites to the no-`orgId` inherited helpers) and verify postgres-persistence (was 435/0).
