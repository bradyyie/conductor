...
2. **Phase 3 (Enterprise) — prove the full vertical.** Convert **one** Orkes store (postgres `ExecutionDAO`) from a standalone re-implementation to `extends` the OSS `PostgresExecutionDAO`, overriding `applyQueryExtensions`/`applyWriteExtensions` to inject `org_id` (read filter + insert column + `ON CONFLICT` target via `OrgContext`); **delete the duplicated SQL**. ✅ The DAO half is **DONE** (§16 Phase 3). The engine half — re-express `OrkesWorkflowExecutor` as `extends WorkflowExecutorOps` — is the **single largest remaining convergence item** and is now concretely scoped (2026-06-26):
   ...
   - **▶ STEP A2 backport inventory (per-method diff, 2026-06-26) — the non-enterprise CORE-DIFFs to port Orkes→OSS**, in priority/risk order (✅=done):
     ...
     - **▶ Backport PROGRESS (2026-06-26):** ✅ **#1 `scheduleNextIteration`** (loopTaskId stamp) and ✅ **#2 `findLastFailedSubWorkflowIfAny`** (DO_WHILE descent) backported to OSS `WorkflowExecutorOps` — OSS engine tests green (`TestWorkflowExecutor` 55/0, `DecideLoop` 2/0, `SeamsTest` 2/0); committed (OSS `4226914fb`). ⏸ **Deferred (need more than a mechanical port):** #4 `terminate` `$`-resolution (OSS `ParametersUtils` lacks `replaceString` — must port that first; existing failureWorkflow test = semantic-change risk); #8 `skipTaskFromWorkflow` composite-children skip (`collectTasks()` self-duplication semantics + no existing OSS composite-skip test — needs a dedicated test). ⌛ **Pending:** #3 (updateTask latency), #5 (restart overrides), #6 (decide-queue hygiene), #7 (double-trigger guard), #9 (updateAndPushParents parent reactivation — highest value, larger), #10 (resumeWorkflow + `WorkflowRepairer` port), and the optional rerun-engine adoption. Then: structural `extends` prep + Orkes rewrite + integration testing. Each remaining item is its own tested increment (critical path; never batch-and-pray).
